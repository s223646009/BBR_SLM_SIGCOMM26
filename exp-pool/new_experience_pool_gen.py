...
# (location,t,bps,retransmits,snd_cwnd,snd_wnd,rtt,rttvar,phase,S_selected,best_action,random_action,
#  reward_random,best_action_reward_based,best_gain_reward_based,reward_best)


df = pd.read_csv("D:\Rakshitha De Silva/1LLM\exp-pool\processed_bbr_data.csv")
...
for index, row in df_training.iterrows():
    ...
    reward = row["reward_random"]
    action = row['random_action']
    exp_pool_training.add(state=state, action=action, reward=reward, done=0)

pickle_save_path = "../llm_framework/data/exp_pools/new_bbr_exp_pool.pkl" # Training data
...
pickle_save_path = "../llm_framework/data/exp_pools/new_bbr_exp_pool_test.pkl" # Evaluating data
pickle.dump(exp_pool_testing, open(pickle_save_path, 'wb'))
print(f"Done. Evaluating Experience pool saved at:", pickle_save_path)